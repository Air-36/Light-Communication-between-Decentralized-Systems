...
// ------------ BUTTON ------------
#define BUTTON_PIN 7
#define CLEAR_BUTTON_PIN 6   // NEW: button to clear existing file
...
void setup() {
  ...
  pinMode(CLEAR_BUTTON_PIN, INPUT_PULLUP); // NEW
...
void loop() {
  ...
  // ------------------------------------------------------------
  //            CLEAR FILE BUTTON: Truncate existing file
  // ------------------------------------------------------------
  if (digitalRead(CLEAR_BUTTON_PIN) == LOW) {
    ...
    // Only truncate if file already exists — DO NOT create a new file
    if (SD.exists("shri.txt")) {
      File f = SD.open("shri.txt", FILE_WRITE);
      if (f) {
        // Opening with FILE_WRITE and immediately closing truncates to zero bytes
        f.close();
        Serial.println("shri.txt truncated (cleared).");
      } else {
        Serial.println("Failed to open shri.txt for truncation!");
      }
    } else {
      Serial.println("shri.txt does not exist — nothing to clear.");
    }
    ...
    // optional: show current content (will show "Empty" if truncated / empty)
    displaySDContent();
  }

  // ------------------------------------------------------------
  //            BUTTON PRESS: CREATE FILE IF MISSING (existing behavior)
  // ------------------------------------------------------------
...
//      File f = SD.open("shri.txt", FILE_WRITE);
//      if (f) {
//        f.close();   // empty file created
//        Serial.println("Created empty shri.txt");
//      }
//    }
