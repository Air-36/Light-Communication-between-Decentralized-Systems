...
// ------------------------------------------------------
//       FILE CREATION HELPER — NEWLY ADDED
// ------------------------------------------------------
...
void setup() {
  ...
  // ---- NEW: auto-create file if absent ----
  ensureFileExists("char_store.txt");
...
void loop() {
  ...
  if (ready) {
    ...
    logFile = SD.open("char_store.txt", FILE_WRITE);
    if (logFile) {
      logFile.print(b);
      logFile.flush();
      logFile.close();
...
void displaySDContent() {
  ...
  logFile = SD.open("char_store.txt");
  if (!logFile) {
    digitalWrite(__CS, LOW);
    digitalWrite(__DC, LOW);
