...
if (digitalRead(CLR_PIN) == LOW) {
    ...
    if (SD.exists("shri.txt")) {
        SD.remove("shri.txt");               // delete old file
        File f = SD.open("shri.txt", FILE_WRITE);
        if (f) {
            f.close();                       // recreate empty file
  ...
  if (digitalRead(BUTTON_PIN) == LOW) {
    ...
    // Toggle between SD mode and humidity mode
...
if (showHumidity) {
    int rawHumid = analogRead(humidityPin);
    float humidity = rawHumid * 100.0 / 1023.0;

    tft.fillRect(0, 60, 128, 40, BLACK);  // clear old value
